  ...
  return (
    <>
      {calculatedHistory && calculatedHistory.length > 0 ? (
        <div className="w-full px-3 py-5 space-y-6">
          {calculatedHistory.map((data, index) => {
            return (
              <div
                key={index}
                className="relative flex flex-col items-center px-2 pt-20 pb-5 bg-blue-200 rounded-md md:pt-14 "
              >
                <div className="flex items-center">
                  {changeTitleTarget && data.id === changeTitleTarget ? (
                    ...
                    >
                      <input className="p-2 h-7" {...register("changeTitle")} />
                ...
                <span className="mb-6 text-xs">{data.date}</span>
                ...
                <div className="flex justify-center w-full">
                  <div
                    className={classNameHandler(
                      data.calculatedProducts.length > 4
                        ? " grid-cols-3"
                        : "grid-cols-3",
                      "grid gap-2 p-2 mb-5 "
                    )}
                    ...
                    {data.calculatedProducts.map(
                      (productData, productIndex) => {
                        return (
                          <div
                            key={productIndex}
                            className="flex flex-col items-center justify-between px-2 py-4 bg-blue-100 rounded-md "
                          >
                            <div className="flex flex-col items-center">
                              ...
                              <span className="mb-2 text-xs text-center break-keep">
                                {productData.name}
                              </span>
                            </div>
